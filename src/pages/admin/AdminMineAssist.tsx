...
export const AdminMineAssist: React.FC = () => {
  ...
  const adminPromptChips = [
    ...
    "Summarize statutory DGMS safety rules."
  ];
    ...
    <div className="space-y-4 flex flex-col h-[calc(100vh-12rem)] min-h-[550px]">
      ...
      <div className="bg-white border border-[#E2DCD0] rounded-lg p-4 shadow-xs flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-md bg-[#102A43] text-white flex items-center justify-center shadow-xs">
            <Shield className="w-5 h-5 text-[#F59E0B]" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-bold text-[#102A43]">
                {t.navMineAssist} • Enterprise AI Retrieval Engine
              </h1>
              <span className="text-[10px] uppercase font-mono font-bold px-1.5 py-0.5 rounded bg-amber-50 border border-amber-200 text-amber-900">
                HQ Authorization
              </span>
            </div>
            <p className="text-xs text-[#64748B]">
              Natural language cross-subsidiary search across 257+ indexed technical reports, borehole logs, and compliance filings
            </p>
          </div>
        </div>
      ...
      <PrototypeNotice 
        message="Enterprise RAG Query Engine: Synthesis grounded across all 6 coalfield clusters and DGMS Master Circulars."
      />
      ...
      <div className="flex-1 bg-white border border-[#E2DCD0] rounded-lg p-4 sm:p-5 overflow-y-auto space-y-4 shadow-xs">
        {chatMessages.map(msg => (
          ...
          >
            {msg.role === 'assistant' && (
              <div className="w-7 h-7 rounded-full bg-[#102A43] text-white flex items-center justify-center shrink-0 mt-0.5 shadow-xs">
                <Bot className="w-4 h-4" />
              </div>
            )}

            <div
              className={`max-w-2xl rounded-lg p-4 space-y-2.5 ${
                msg.role === 'user'
            ...
            >
              ...
              {msg.citations && msg.citations.length > 0 && (
                <div className="pt-2.5 border-t border-[#EAE5DC] text-[11px] text-[#64748B] space-y-1.5">
                  <span className="font-semibold text-[#102A43] block uppercase tracking-wider text-[10px]">
                    Authoritative Citations & Document Lake References:
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                    {msg.citations.map((c, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white p-1.5 rounded border border-[#E2DCD0]">
                        <FileText className="w-3.5 h-3.5 text-[#102A43] shrink-0" />
                        <div className="min-w-0">
                          <span className="font-semibold text-[#102A43] block truncate">{c.title}</span>
                          <span className="text-[10px] text-[#94A3B8] block truncate">{c.ref}</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {msg.role === 'user' && (
              <div className="w-7 h-7 rounded-full bg-[#EAE5DC] text-[#102A43] flex items-center justify-center shrink-0 mt-0.5">
                <Shield className="w-4 h-4 text-[#102A43]" />
              </div>
            )}
          </div>
        ))}
      ...
      <form onSubmit={handleSend} className="bg-white border border-[#E2DCD0] rounded-lg p-2 flex items-center gap-2 shadow-xs shrink-0">
        <input
          ...
          placeholder="Ask across all 6 coalfields: e.g., 'Summarize pending documents for Rajmahal OCP'..."
