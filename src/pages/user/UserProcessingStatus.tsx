...
export const UserProcessingStatus: React.FC = () => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-[#102A43] tracking-tight mt-0.5">
              Document Digitization Pipeline Monitor
            </h1>
            <p className="text-xs text-[#64748B] mt-1">
              Live automated optical character recognition (OCR), table extraction, and geological schema tagging
            </p>
          </div>

          <div className="text-xs font-semibold px-3 py-1.5 rounded bg-[#FAF9F6] border border-[#E2DCD0] text-[#102A43]">
            Active Jobs: {processingJobs.filter(j => j.status !== 'processed').length} running
          </div>
        </div>
      </div>

      <PrototypeNotice 
        message="Demonstration Processing Engine: Click 'Advance Pipeline' on any active job to simulate neural OCR progress and JSON schema extraction." 
      />
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  {job.status !== 'processed' && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-md bg-[#102A43] text-white font-semibold hover:bg-[#1E3A8A] transition-colors flex items-center gap-1.5 shadow-xs"
                    >
              ...
              <div>
                <div className="flex justify-between text-[11px] font-medium text-[#64748B] mb-1.5">
                  <span>Digitization Completeness</span>
              ...
              <div className="pt-2 border-t border-[#F1EFEA] flex flex-wrap justify-between text-[11px] text-[#64748B]">
                ...
                <span>Last Processed: <strong>{job.lastUpdatedTime}</strong></span>
                <span>Pipeline Engine: <strong>PyTesseract + Gemini Document AI Adapter</strong></span>
              </div>
            </div>
          );
        })}
